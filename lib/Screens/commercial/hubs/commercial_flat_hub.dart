...
class CommercialFlatHubForm extends StatelessWidget {
  ...
  const CommercialFlatHubForm({
    ...
    required this.removePhotos,
      ...
      roof.roofType == 'TPO' ||
      roof.roofType == 'EPDM' ||
      roof.roofType == 'Modified Bitumen';
  ...
  Widget build(BuildContext context) {
    ...
    TextField(
      decoration: const InputDecoration(
        labelText: 'Specify insulation material *',
        border: OutlineInputBorder(),
        hintText: 'Ej: Polyiso, Foam, etc.',
      ),
      onChanged: (value) {
        roof.insulationMaterialOtherSpecify = value.trim().isEmpty ? null : value.trim();
        sync(); // Para guardar en tiempo real
      },
    ),
  ],
          const SizedBox(height: 12),
